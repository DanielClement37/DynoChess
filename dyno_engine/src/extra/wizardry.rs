...
pub fn find_magics(piece: Piece) {
    ...
    for sq in RangeOf::SQUARES {
        ...
        let r_mask = MoveGen::rook_mask(sq);
        let b_mask = MoveGen::bishop_mask(sq);
        ...
        let blocker_boards = MoveGen::blocker_boards(mask);

        // Create attack boards for the current square/blocker combo (either
        // rook or bishop).
        let r_ab = MoveGen::rook_attack_boards(sq, &blocker_boards);
        let b_ab = MoveGen::bishop_attack_boards(sq, &blocker_boards);
