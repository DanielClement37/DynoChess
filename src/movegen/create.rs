...
impl MoveGen {
    ...
    pub fn rook_mask(square: Square) -> Bitboard {
        let location = Board::square_location(square);
        let bb_rook_square = BB_SQUARES[square];
        let bb_edges = MoveGen::edges_without_piece(location);
    ...
    pub fn bishop_mask(square: Square) -> Bitboard {
        let location = Board::square_location(square);
        let bb_edges = MoveGen::edges_without_piece(location);
        let bb_up_left = MoveGen::bb_ray(0, square, Direction::UpLeft);
        let bb_up_right = MoveGen::bb_ray(0, square, Direction::UpRight);
        let bb_down_right = MoveGen::bb_ray(0, square, Direction::DownRight);
        let bb_down_left = MoveGen::bb_ray(0, square, Direction::DownLeft);
    ...
    pub fn rook_attack_boards(square: Square, blockers: &[Bitboard]) -> AttackBoards {
        ...
        for b in blockers.iter() {
            let bb_attacks = MoveGen::bb_ray(*b, square, Direction::Up)
                | MoveGen::bb_ray(*b, square, Direction::Right)
                | MoveGen::bb_ray(*b, square, Direction::Down)
                | MoveGen::bb_ray(*b, square, Direction::Left);
    ...
    pub fn bishop_attack_boards(square: Square, blockers: &[Bitboard]) -> AttackBoards {
        ...
        for b in blockers.iter() {
            let bb_attacks = MoveGen::bb_ray(*b, square, Direction::UpLeft)
                | MoveGen::bb_ray(*b, square, Direction::UpRight)
                | MoveGen::bb_ray(*b, square, Direction::DownRight)
                | MoveGen::bb_ray(*b, square, Direction::DownLeft);
            bb_attack_boards.push(bb_attacks);
        }
    ...
    pub fn bb_ray(bb_in: Bitboard, square: Square, direction: Direction) -> Bitboard {
        let mut file = Board::square_location(square).0 as usize;
        let mut rank = Board::square_location(square).1 as usize;
